...
must_haves:
  ...
  artifacts:
    ...
    - path: "backend/monitoring/checks.py"
      provides: "Health check implementations for backend, frontend, API, database, external services"
      exports: ["run_health_checks", "CheckResult"]
...
<objective>
Create the monitoring service foundation: metrics database, health check implementations, and scheduled check runner.
...
Output: A standalone monitoring service (`backend/monitoring/`) with SQLite metrics DB, health checks for all critical systems, and APScheduler running checks on intervals.
</objective>
...
Run `python -c "from monitoring.storage import init_db, store_metric, get_recent_metrics; init_db(); store_metric('test', 'pass', 100.0); print(get_recent_metrics('test'))"` from backend/ directory.
  ...
  <done>Monitoring config loads from environment, metrics DB creates tables on init, metrics can be stored and queried, CheckResult dataclass is importable.</done>
</task>

<task type="auto">
  <name>Task 2: Health checks and APScheduler runner</name>
  <files>
    backend/monitoring/checks.py
    backend/monitoring/scheduler.py
    backend/monitoring/main.py
...
1. `python -c "from monitoring.checks import check_backend_health, CheckResult; print('Checks import OK')"` succeeds
...
3. `python -c "from monitoring.main import app; print(f'App routes: {[r.path for r in app.routes]}')"` shows expected routes
  ...
  <done>Health checks detect backend/frontend/API failures. Scheduler runs checks at configured intervals. Monitoring FastAPI app serves metrics via API on port 8001. All check results stored in SQLite metrics DB.</done>
</task>
...
- Scheduler has jobs configured at correct intervals
</verification>
...
- APScheduler runs checks at configured intervals (1min health, 5min beta, 10min weather)
- Metrics API endpoints serve check history and uptime stats
- Incident tracking (create/resolve) works
</success_criteria>
