...
must_haves:
  ...
  artifacts:
    - path: "backend/monitoring/alerts/manager.py"
      provides: "Alert evaluation, deduplication, escalation logic, acknowledgment handling"
  ...
  key_links:
    ...
    - from: "backend/monitoring/alerts/manager.py"
      ...
      via: "reads consecutive failures, creates/resolves/acknowledges incidents"
      pattern: "get_consecutive_failures|store_incident|resolve_incident|acknowledge_incident"
---

<objective>
Build the alert management system with SMS and email channels, deduplication, severity routing, escalation, and incident acknowledgment.

Purpose: Transform check results into actionable alerts. Without this, checks run but nobody is notified when things break. This is the most critical piece for catching issues like the BetaApplyModal bug. Acknowledgment prevents escalation spam when a responder is already investigating.

Output: AlertManager that evaluates check results, deduplicates alerts, routes to SMS/email based on severity, sends recovery notifications, and supports incident acknowledgment to stop escalation.
</objective>
...
<task type="auto">
  <name>Task 2: Alert manager with deduplication, escalation, acknowledgment, and scheduler wiring</name>
  <files>
    backend/monitoring/alerts/manager.py
    backend/monitoring/scheduler.py
  </files>
...
- `async evaluate_and_alert(result: CheckResult)`:
  ...
  3. If result.status == 'fail':
     ...
     - Determine severity based on check_name mapping:
       - Critical (immediate SMS + email): `health_check`, `beta_signup_flow`, `checkout_flow`, `sms_webhook`
       - Warning (email, SMS after 15min): `login_flow`, `weather_api`, `api_response_time`, `db_query_performance`, `external_api_latency`
...
- `_should_escalate(check_name: str) -> bool`:
  - Check if incident is acknowledged - if so, do NOT escalate (acknowledgment stops escalation)
  - Check if incident has been active > 15 minutes AND no SMS sent yet
  - Return True only if unacknowledged AND time threshold met
...
- `_determine_severity(check_name: str) -> str`:
  ...
  - Default: critical for health/beta/checkout/sms_webhook, warning for others
...
- `_send_warning_alert(check_name, result, incident)`:
  ...
  - If incident.first_seen > 15 minutes ago AND no SMS sent AND incident NOT acknowledged: escalate to SMS
...
- `acknowledge_incident(incident_id: str)`:
  - Call storage.acknowledge_incident(incident_id) to set status='acknowledged'
  - Log acknowledgment with timestamp
  - This prevents further escalation (warning->SMS) for this incident
  - Does NOT stop the incident from being tracked or resolved
...
Verify scheduler.py imports and calls evaluate_and_alert: `grep -n 'evaluate_and_alert' backend/monitoring/scheduler.py` shows integration.
  ...
  <done>Alert manager evaluates check results with deduplication (15-min window), requires 2 consecutive failures before alerting, routes critical alerts to SMS+email and warnings to email with 15-min escalation, sends recovery notifications, supports incident acknowledgment to stop escalation, and is wired into the scheduler's check execution loop.</done>
</task>
...
- Critical checks (health, beta, checkout, sms_webhook) route to SMS + email
- Warning checks (login, weather, db_query, external_api) route to email with SMS escalation
- Acknowledged incidents do NOT escalate (no warning->SMS escalation)
...
- Warning failure persisting 15+ minutes -> escalated from email to SMS (unless acknowledged)
- Acknowledged incident -> no further escalation, still tracked until resolved
