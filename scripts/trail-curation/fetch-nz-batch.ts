// Fetch New Zealand trail data from OSM Overpass with role-aware filtering + way chaining
// Usage: npx tsx scripts/trail-curation/fetch-nz-batch.ts [--start N] [--end N] [--resume] [--skip-over N]
//
// Pipeline:
...
const ID_OVERRIDES: Record<string, string> = {
  'roy_s_peak_track': 'roys_peak_track',
  'rees_dart_track': 'reesdart_track',
};
...
async function main() {
  ...
  const trails: TrailInput[] = JSON.parse(
    readFileSync(join(process.cwd(), 'scripts', 'trail-curation', 'trail-lists', 'nz-trails.json'), 'utf-8')
  );
  ...
  for (let i = 0; i < subset.length; i++) {
    ...
    // Save progress
    writeFileSync(
      join(resultsDir, 'nz-batch-progress.json'),
