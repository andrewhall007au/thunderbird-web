// Fix pass for European mega trail data
// Applies: flyback removal, stitch two halves, gap skipping, section dedup,
//          out-and-back mirroring, E5 lat cutoff, Kungsleden stage dedup
// Key rule: ONLY apply a correction if it brings calculated distance CLOSER to official
//
// Usage: npx tsx scripts/trail-curation/fix-europe-mega.ts [--dry-run]
...
// --- Special Fix: E5 latitude cutoff (Oberstdorf to Meran only) ---
function e5LatCutoff(coords: [number, number, number][]): { coords: [number, number, number][]; description: string } {
  // E5 full route goes DE→AT→IT all the way to Verona
  // We only want Oberstdorf (~47.37°N) to Meran/Merano (~46.67°N)
  // Cut anything south of 46.5°N (buffer below Meran)
  const SOUTH_CUTOFF = 46.5;
  // Also cut anything north of 47.5°N (buffer above Oberstdorf)
  const NORTH_CUTOFF = 47.5;

  const filtered = coords.filter(c => c[1] >= SOUTH_CUTOFF && c[1] <= NORTH_CUTOFF);
  const removed = coords.length - filtered.length;

  return {
    coords: filtered,
    description: `E5 lat cutoff: removed ${removed} points outside ${SOUTH_CUTOFF}°N-${NORTH_CUTOFF}°N`,
  };
}

// --- Special Fix: Kungsleden stage dedup ---
// Kungsleden has duplicate out-and-back stages (Kebnekaise side trip mapped both ways)
function kungsledenDedup(coords: [number, number, number][]): { coords: [number, number, number][]; description: string } {
  // Split into sections at gaps, then detect overlapping sections
  const sections = splitAtGaps(coords, 1);
  if (sections.length <= 1) return { coords, description: 'no sections to dedup' };

  // Track section centroids to detect duplicates
  const kept: [number, number, number][][] = [];
  const centroids: [number, number][] = [];

  for (const sec of sections) {
    if (sec.length < 5) continue; // skip tiny fragments

    const centLat = sec.reduce((s, c) => s + c[1], 0) / sec.length;
    const centLon = sec.reduce((s, c) => s + c[0], 0) / sec.length;

    // Check if any existing section has a similar centroid
    let isDup = false;
    for (const [cLat, cLon] of centroids) {
      if (haversine(centLat, centLon, cLat, cLon) < 3) { // within 3km = likely same section
        isDup = true;
        break;
      }
    }
    ...
    if (!isDup) {
      kept.push(sec);
      centroids.push([centLat, centLon]);
    }
  }

  const removed = sections.length - kept.length;
  if (removed === 0) return { coords, description: 'no duplicates found' };

  return {
    coords: kept.flat() as [number, number, number][],
    description: `deduped ${removed} duplicate sections (${sections.length} → ${kept.length})`,
  };
}
...
function tryStitchHalves(coords: [number, number, number][], officialKm: number): { coords: [number, number, number][]; stitched: boolean; description: string } {
  ...
  if (gapToOrigin < gapToS1End && gapToOrigin < 5) {
    ...
    return {
      ...
      stitched: true,
      description: `stitched ${options[0].desc}: s1=${s1Km.toFixed(1)}km + s2=${s2Km.toFixed(1)}km → ${options[0].km.toFixed(1)}km`,
    };
  }
...
async function main() {
  ...
  console.log(`=== FIX EUROPE MEGA TRAILS ===${dryRun ? ' (DRY RUN)' : ''}\n`);

  const progressPath = join(process.cwd(), 'scripts', 'trail-curation', 'results', 'europe-mega-progress.json');
  console.log(`Reading from: europe-mega-progress.json\n`);
  ...
  for (const r of progress.results) {
    ...
    if (!existsSync(filePath)) {
      console.log(`SKIP ${r.name}: file not found`);
      continue;
    }
    ...
    console.log(`\n--- ${trail.name} ---`);
    console.log(`  Before: ${beforeKm.toFixed(1)}km (official ${officialKm}km, ${(beforeErr * 100).toFixed(1)}% off)`);
    ...
    // E5: Latitude cutoff (Oberstdorf to Meran only)
    if (r.id === 'e5_alps_crossing') {
      const cutoff = e5LatCutoff(trail.coordinates);
      if (cutoff.coords.length < trail.coordinates.length) {
        const cutKm = totalDist(cutoff.coords);
        const cutErr = Math.abs(cutKm - officialKm) / officialKm;
        console.log(`  ${cutoff.description}`);
        console.log(`  After cutoff: ${cutKm.toFixed(1)}km (${(cutErr * 100).toFixed(1)}% off)`);
        if (cutErr < bestErr) {
          bestCoords = cutoff.coords;
          bestKm = cutKm;
          bestErr = cutErr;
          action = 'E5 lat cutoff';
        }
      }
    }

    // Kungsleden: Stage dedup
    if (r.id === 'kungsleden') {
      const dedup = kungsledenDedup(trail.coordinates);
      if (dedup.coords.length < trail.coordinates.length) {
        const dedupKm = totalDist(dedup.coords);
        const dedupErr = Math.abs(dedupKm - officialKm) / officialKm;
        console.log(`  ${dedup.description}`);
        console.log(`  After dedup: ${dedupKm.toFixed(1)}km (${(dedupErr * 100).toFixed(1)}% off)`);
        if (dedupErr < bestErr) {
          bestCoords = dedup.coords;
          bestKm = dedupKm;
          bestErr = dedupErr;
          action = 'Kungsleden stage dedup';
        }
      }
    }

    // === GENERIC FIXES (applied to all, including post-special-fix coords) ===

    const sections = splitAtGaps(bestCoords, 2);
    ...
    if (sections.length > 1) {
      ...
      if (removed > 0) {
        ...
        if (flybackErr < bestErr && flybackCoords.length > bestCoords.length * 0.3) {
          bestCoords = flybackCoords;
          bestKm = flybackKm;
          bestErr = flybackErr;
          action = action === 'no change'
            ? `removed ${removed} flyback section(s)`
            : `${action} + removed ${removed} flyback(s)`;
        }
      }
    }

    // --- Try Fix 2: Stitch two halves ---
    const sectionsPost = splitAtGaps(bestCoords, 2);
    if (sectionsPost.length === 2) {
      const stitch = tryStitchHalves(bestCoords, officialKm);
      if (stitch.stitched) {
        ...
        if (stitchErr < bestErr) {
          ...
          bestKm = stitchKm;
          bestErr = stitchErr;
          action = action === 'no change' ? stitch.description : `${action} + ${stitch.description}`;
        }
      }
    }

    // --- Try Fix 3: Section dedup ---
    const sectionsPost2 = splitAtGaps(bestCoords, 2);
    if (sectionsPost2.length > 1) {
      const dedup = sectionDedup(bestCoords, officialKm);
      if (dedup.deduped) {
        const dedupKm = totalDist(dedup.coords);
        const dedupErr = Math.abs(dedupKm - officialKm) / officialKm;
        if (dedupErr < bestErr) {
          ...
          bestKm = dedupKm;
          bestErr = dedupErr;
          action = action === 'no change' ? 'section dedup' : `${action} + section dedup`;
        }
      }
    }
    ...
    if (halfOff < 0.15 && bestErr > 0.3) {
      ...
      if (mirroredErr < bestErr) {
        ...
        bestErr = mirroredErr;
        action = action === 'no change'
          ? `out-and-back mirrored (${beforeKm.toFixed(1)}km → ${mirroredKm.toFixed(1)}km)`
          : `${action} + mirrored`;
      }
    }
    ...
    if (gapResult.threshold > 0 && Math.abs(gapResult.calcKm - officialKm) / officialKm < bestErr) {
      ...
      bestErr = Math.abs(gapResult.calcKm - officialKm) / officialKm;
    }
    ...
    if (action !== 'no change') {
      ...
      if (!dryRun) {
        ...
        if (existsSync(manifestPath)) {
          ...
          const idx = manifest.findIndex((m: any) => m.id === trail.id);
          if (idx >= 0) {
            manifest[idx].calculatedKm = bestKm;
            manifest[idx].pointCount = bestCoords.length;
  ...
  // Summary table
  console.log('\n=== QUALITY SUMMARY ===');
  console.log(`| Status | Trail | Calc | Official | Off | Action |`);
  console.log(`|--------|-------|------|----------|-----|--------|`);
  ...
  for (const r of sorted) {
    ...
    console.log(`| ${status} | ${r.name} | ${r.after.toFixed(1)}km | ${r.official}km | ${(err * 100).toFixed(1)}% | ${r.action} |`);
  }
