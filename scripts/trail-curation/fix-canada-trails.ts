// Combined fix pass for Canadian trail data
// Applies: flyback removal, stitch two halves, gap skipping, out-and-back mirroring
// Key rule: ONLY apply a correction if it brings calculated distance CLOSER to official
//
// Usage: npx tsx scripts/trail-curation/fix-canada-trails.ts [--dry-run]
...
// --- Fix 3: Adaptive gap skipping (calculate distance excluding large gaps) ---
...
// --- Fix 4: Section dedup (Lost Coast pattern) ---
function sectionDedup(coords: [number, number, number][], officialKm: number): { coords: [number, number, number][]; deduped: boolean } {
  ...
  const origin = sections[0][0];
  // Find sections that start near origin
  ...
  // Keep the section whose distance best matches official
...
async function main() {
  ...
  console.log(`=== FIX CANADA TRAILS ===${dryRun ? ' (DRY RUN)' : ''}\n`);

  // Load Canada trail list for official distances
  const trailList = JSON.parse(
    readFileSync(join(process.cwd(), 'scripts', 'trail-curation', 'trail-lists', 'canada-trails.json'), 'utf-8')
  );

  // Load progress file to get the IDs we actually fetched
  const sourceArg = process.argv.find(a => a.startsWith('--source='));
  const progressFile = sourceArg ? sourceArg.split('=')[1] : 'canada-batch-progress.json';
  const progressPath = join(process.cwd(), 'scripts', 'trail-curation', 'results', progressFile);
  console.log(`Reading from: ${progressFile}\n`);
  ...
  for (const r of progress.results) {
    ...
    // Only if current calc ≈ official/2
    ...
    if (gapResult.threshold > 0 && Math.abs(gapResult.calcKm - officialKm) / officialKm < bestErr) {
      ...
      bestErr = gapErr;
      // Note: gap skip doesn't change coordinates, just the stored calculatedKm
    }
