...
function removeFlybacks(sections: [number, number, number][][]): { kept: [number, number, number][][]; removed: number } {
  ...
  for (let i = 1; i < sections.length; i++) {
    ...
    if (isFlyback) { removed++; } else { kept.push(sections[i]); }
  }
  return { kept, removed };
}
...
function tryStitchHalves(coords: [number, number, number][], officialKm: number): { coords: [number, number, number][]; stitched: boolean; description: string } {
  ...
  const s1 = sections[0], s2 = sections[1];
  const origin = s1[0], s1End = s1[s1.length - 1], s2Start = s2[0];
  ...
  if (gapToOrigin < gapToS1End && gapToOrigin < 5) {
    ...
    return { coords: options[0].coords as [number, number, number][], stitched: true, description: `stitched ${options[0].desc}: ${options[0].km.toFixed(1)}km` };
  }
  if (gapToS1End < 10) {
    return { coords: [...s1, ...s2] as [number, number, number][], stitched: true, description: `gap-stitched: ${gapToS1End.toFixed(1)}km gap` };
  }
...
function adaptiveGapSkip(coords: [number, number, number][], officialKm: number): { calcKm: number; threshold: number } {
  ...
  for (const t of thresholds) {
    ...
    if (err < bestErr) { bestKm = d; bestThreshold = t; bestErr = err; }
  }
...
function sectionDedup(coords: [number, number, number][], officialKm: number): { coords: [number, number, number][]; deduped: boolean } {
  ...
  for (const s of nearOrigin) {
    ...
    if (err < bestErr) { bestErr = err; bestSection = s; }
  }
...
async function main() {
  ...
  console.log(`=== FIX INDIA/HIMALAYAS TRAILS ===${dryRun ? ' (DRY RUN)' : ''}\n`);

  const progressPath = join(process.cwd(), 'scripts', 'trail-curation', 'results', 'india-himalayas-batch-progress.json');
  const progress = JSON.parse(readFileSync(progressPath, 'utf-8'));

  // Delete garbage data first
  const deleteThreshold = 50; // % off
  const deleteMinPts = 200;   // or fewer than 200 points
  const toDelete: string[] = [];

  for (const r of progress.results) {
    if (r.off > deleteThreshold && r.pts < deleteMinPts) {
      toDelete.push(r.id);
      console.log(`DELETING: ${r.name} (${r.pts} pts, ${r.off.toFixed(1)}% off) — garbage data`);
      if (!dryRun) {
        ...
        if (existsSync(manifestPath)) {
          ...
          const filtered = manifest.filter((m: any) => m.id !== r.id);
  ...
  for (const r of progress.results) {
    ...
    let trail: TrailData;
    try { trail = JSON.parse(readFileSync(filePath, 'utf-8')); } catch { continue; }
    ...
    if (sections.length > 1) {
      ...
      if (removed > 0) {
        ...
        if (flybackErr < bestErr && flybackCoords.length > trail.coordinates.length * 0.3) {
          bestCoords = flybackCoords; bestKm = flybackKm; bestErr = flybackErr;
    ...
    if (sections.length === 2) {
      ...
      if (stitch.stitched) {
        ...
        if (stitchErr < bestErr) {
          bestCoords = stitch.coords; bestKm = stitchKm; bestErr = stitchErr;
    ...
    if (sections.length > 1) {
      ...
      if (dedup.deduped) {
        ...
        if (dedupErr < bestErr) {
          bestCoords = dedup.coords; bestKm = dedupKm; bestErr = dedupErr;
          action = 'section dedup';
        }
      }
    }

    // Fix 4: Out-and-back mirroring
    ...
    if (halfOff < 0.15 && bestErr > 0.3) {
      ...
      if (mirroredErr < bestErr) {
        bestCoords = mirrored; bestKm = mirroredKm; bestErr = mirroredErr;
        action = `out-and-back mirrored (${beforeKm.toFixed(1)}km → ${mirroredKm.toFixed(1)}km)`;
      }
    }

    // Fix 5: Adaptive gap skip
    ...
    if (gapResult.threshold > 0 && Math.abs(gapResult.calcKm - officialKm) / officialKm < bestErr) {
      if (action === 'no change') { action = `gap-skip threshold=${gapResult.threshold}km`; }
      else { action += ` + gap-skip ${gapResult.threshold}km`; }
      bestKm = gapResult.calcKm; bestErr = Math.abs(gapResult.calcKm - officialKm) / officialKm;
    }
  ...
  for (const r of sorted) {
    ...
    if (status === 'OK') okCount++; else if (status === 'FAIR') fairCount++; else highCount++;
    const changed = r.action !== 'no change' ? ' *' : '';
    console.log(`| ${status} | ${r.name} | ${r.after.toFixed(1)}km | ${r.official}km | ${(err * 100).toFixed(1)}%${changed} |`);
  }
  console.log(`\nDeleted: ${toDelete.length} garbage trails`);
  console.log(`Fixed: ${fixedCount}/${results.length} trails`);
