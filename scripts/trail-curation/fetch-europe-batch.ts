// Fetch European trail data from OSM Overpass with role-aware filtering + way chaining
// Usage: npx tsx scripts/trail-curation/fetch-europe-batch.ts [--start N] [--end N] [--resume] [--skip-over N]
//
// Pipeline:
...
// --- Coordinate validation (broad Europe: lat 35-70, lon -25 to 45) ---
...
async function fetchSingleTrail(input: TrailInput): Promise<{
  ...
  // Verify coordinates are in Europe
  const midIdx = Math.floor(coords2d.length / 2);
  if (!isInEurope(coords2d[midIdx][1], coords2d[midIdx][0])) {
    console.log(`  WARNING: Coordinates not in Europe (${coords2d[midIdx][1].toFixed(2)}, ${coords2d[midIdx][0].toFixed(2)}) — skipping`);
    return null;
  }
...
async function main() {
  ...
  console.log(`=== FETCH EUROPE TRAILS ===`);
  ...
  const progressPath = join(resultsDir, 'europe-batch-progress.json');
