// Fetch Canadian trail data from OSM Overpass with role-aware filtering + way chaining
// Usage: npx tsx scripts/trail-curation/fetch-canada-batch.ts [--start N] [--end N] [--resume] [--skip-over N]
//
// Pipeline:
...
// --- ID overrides for trails whose toId() doesn't match popularTrails.ts ---

const ID_OVERRIDES: Record<string, string> = {
  'bruce_trail_bruce_peninsula': 'bruce_trail_bruce_peninsula',
  'la_cloche_silhouette_trail': 'la_cloche_silhouette_trail',
  'coastal_trail_pukaskwa': 'coastal_trail_pukaskwa',
  'killarney_provincial_park_the_crack': 'killarney_provincial_park_the_crack',
  'sentier_des_caps_de_charlevoix': 'sentier_des_caps_de_charlevoix',
  'sentier_international_des_appalaches': 'sentier_international_des_appalaches',
  'skyline_trail_jasper': 'skyline_trail_jasper',
  'skyline_trail_cape_breton': 'skyline_trail_cape_breton',
  'mt_assiniboine_via_wonder_pass': 'mt_assiniboine_via_wonder_pass',
  'egypt_lake_via_healy_pass': 'egypt_lake_via_healy_pass',
  'lake_agnes_and_big_beehive_trail': 'lake_agnes_and_big_beehive_trail',
  'tombstone_grizzly_lake_trail': 'tombstone_grizzly_lake_trail',
  'carthew_alderson_trail': 'carthew_alderson_trail',
  'cape_chignecto_coastal_trail': 'cape_chignecto',
  'mont_tremblant_summit_trail': 'mont_tremblant_summit_trail',
  'mont_jacques_cartier_trail': 'mont_jacques_cartier_trail',
  'mont_albert_circuit': 'mont_albert_circuit',
  'les_loups_trail': 'les_loups_trail',
  'algonquin_highlands_backpacking_trail': 'algonquin_highlands_backpacking_trail',
};

function getTrailId(name: string): string {
  const autoId = toId(name);
  return ID_OVERRIDES[autoId] || autoId;
}
...
async function main() {
  ...
  const trails: TrailInput[] = JSON.parse(
    readFileSync(join(process.cwd(), 'scripts', 'trail-curation', 'trail-lists', 'canada-trails.json'), 'utf-8')
  );
  ...
  console.log(`=== FETCH CANADA TRAILS ===`);
  ...
  for (let i = 0; i < subset.length; i++) {
    ...
    const trailId = getTrailId(trail.name);
    ...
    try {
      ...
      if (result) {
        // Override ID to match popularTrails.ts
        result.id = trailId;
    ...
    // Save progress
    writeFileSync(
      join(resultsDir, 'canada-batch-progress.json'),
      JSON.stringify({ results, failures, skipped, lastIndex: globalIdx, timestamp: new Date().toISOString() }, null, 2)
    );
  ...
  if (results.length > 0) {
    ...
    console.log(`|--------|-------|------|----------|------|-----|--------|`);
    for (const r of results) {
      const trail = trails.find(t => getTrailId(t.name) === r.id);
