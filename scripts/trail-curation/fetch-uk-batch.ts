// Fetch UK trail data from OSM Overpass with role-aware filtering + way chaining
// Usage: npx tsx scripts/trail-curation/fetch-uk-batch.ts [--start N] [--end N] [--resume] [--skip-over N]
//
// Pipeline:
...
function isInUK(lat: number, lon: number): boolean {
...
async function fetchSingleTrail(input: TrailInput): Promise<{
  ...
  // Verify coordinates are in UK
  const midIdx = Math.floor(coords2d.length / 2);
  if (!isInUK(coords2d[midIdx][1], coords2d[midIdx][0])) {
    console.log(`  WARNING: Coordinates not in UK (${coords2d[midIdx][1].toFixed(2)}, ${coords2d[midIdx][0].toFixed(2)}) — skipping`);
    return null;
  }
...
async function main() {
  ...
  const progressPath = join(resultsDir, 'uk-batch-progress.json');
