// Fetch trail data from OSM Overpass for any region
// Usage: npx tsx scripts/trail-curation/fetch-region-batch.ts --list japan-trails.json [--start N] [--end N] [--resume] [--skip-over N]
//
// Pipeline:
...
// 5. Enrich elevation via SRTM 30m (or ASTER 30m for lat >= 59.5)
...
const TOPO_SRTM = 'https://api.opentopodata.org/v1/srtm30m';
const TOPO_ASTER = 'https://api.opentopodata.org/v1/aster30m';
...
function searchVariants(searchName: string, trailName: string): string[] {
  ...
  // Handle pipe-separated search names (e.g. "熊野古道|Kumano Kodo")
  for (const part of searchName.split('|')) {
    variants.add(escapeRegex(part.trim()));
  }
...
// --- Coordinate validation (bbox-based) ---

function isInBBox(lat: number, lon: number, bbox: [number, number, number, number], margin = 2): boolean {
...
async function enrichElevation(coords: [number, number][]): Promise<[number, number, number][]> {
  ...
  for (let i = 0; i < coords.length; i += 100) {
    ...
    // Use ASTER for high latitudes (>= 59.5) where SRTM has no coverage
    const avgLat = batch.reduce((sum, c) => sum + c[1], 0) / batch.length;
    const api = avgLat >= 59.5 ? TOPO_ASTER : TOPO_SRTM;
...
async function fetchSingleTrail(input: TrailInput): Promise<{
  ...
  // Verify coordinates are within expected bbox (with 2-degree margin)
  if (input.bbox) {
    const midIdx = Math.floor(coords2d.length / 2);
    if (!isInBBox(coords2d[midIdx][1], coords2d[midIdx][0], input.bbox)) {
      console.log(`  WARNING: Coordinates (${coords2d[midIdx][1].toFixed(2)}, ${coords2d[midIdx][0].toFixed(2)}) outside expected bbox — skipping`);
      return null;
    }
  }
  ...
  // Auto-detect out-and-back (calc ≈ official/2)
...
async function main() {
  ...
  if (listIdx === -1 || !args[listIdx + 1]) {
    console.error('Usage: npx tsx fetch-region-batch.ts --list <trail-list.json> [--start N] [--end N] [--resume] [--skip-over N]');
    process.exit(1);
  }
  const listFile = args[listIdx + 1];
  const regionName = basename(listFile, '.json').replace(/-trails$/, '');
  ...
  const listPath = join(process.cwd(), 'scripts', 'trail-curation', 'trail-lists', listFile);
  if (!existsSync(listPath)) {
    console.error(`Trail list not found: ${listPath}`);
    process.exit(1);
  }

  const trails: TrailInput[] = JSON.parse(readFileSync(listPath, 'utf-8'));
  ...
  console.log(`=== FETCH ${REGION} TRAILS ===`);
  ...
  const progressPath = join(resultsDir, `${regionName}-batch-progress.json`);
  if (resume && existsSync(progressPath)) {
    const prev = JSON.parse(readFileSync(progressPath, 'utf-8'));
    results.push(...(prev.results || []));
  ...
  for (let i = 0; i < subset.length; i++) {
    ...
    try {
      ...
      if (result) {
        ...
        results.push({ id: trailId, name: trail.name, calcKm: result.calculatedKm, pts: result.coordinates.length, source: result.dataSource, off: pctOff, status, country: trail.country });
      } else {
    ...
    // Save progress
    writeFileSync(progressPath, JSON.stringify({ results, failures, skipped, lastIndex: globalIdx, timestamp: new Date().toISOString() }, null, 2));
